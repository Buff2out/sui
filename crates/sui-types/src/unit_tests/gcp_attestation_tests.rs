...
fn test_valid_attestation() {
    ...
    let doc = verify_gcp_attestation(VALID_TOKEN.as_bytes(), &n, &e, NOW_MS).expect("should verify");
...
fn test_oversized_token() {
    ...
    let err = verify_gcp_attestation(&oversized_token, &n, &e, NOW_MS).unwrap_err();
    assert_eq!(
        err,
...
fn test_oversized_modulus() {
    ...
    let token = b"a.b.c";
    let err = verify_gcp_attestation(token, &oversized_n, &e, NOW_MS).unwrap_err();
    assert_eq!(
        err,
...
fn test_real_gcp_payload_claims() {
    let (n, e) = decode_key(KEY_A_N, KEY_A_E);
    let doc = verify_gcp_attestation(REAL_GCP_TOKEN.as_bytes(), &n, &e, NOW_MS)
