...
impl DataStore {
    ...
    /// Get the object at its latest known version, checking the local filesystem first and
    /// falling back to the remote GraphQL endpoint at the forked checkpoint. A successful
    /// remote fetch is cached locally before being returned.
    pub fn get_object(&self, object_id: &ObjectID) -> anyhow::Result<Option<Object>> {
        self.get_latest_object(object_id)
    }

    /// Get the object at a specific version. Checks the local filesystem first, then falls back
    /// to the remote endpoint with a `VersionAtCheckpoint` query against the forked checkpoint.
    /// A successful remote fetch is cached locally before being returned.
