...
mod checked {
    ...
    fn execute_transaction<Mode: ExecutionMode>(
        ...
        // If any phase fails, the error short-circuits via and_then and all errors
        // are handled uniformly by reset_and_charge (see below).
        // On success, charge applies the final payment.
        let mut timings = vec![];
        ...
        let (cost_summary, mut result) = match phase_result {
            Ok(v) => {
                let result: Result<Mode::ExecutionResults, Mode::Error> = Ok(v);
                let cost = gas_charger.charge(temporary_store, &result);
                (cost, result)
            }
            Err(e) => {
                let mut result: Result<Mode::ExecutionResults, Mode::Error> = Err(e);
                let cost = reset_and_charge(gas_charger, temporary_store, &mut result);
                (cost, result)
            }
        };
    ...
    /// Uniform error recovery. Performs two things:
    /// 1. Reset: drop execution writes, re-smash gas, re-touch all mutable input objects.
    ///    After reset the store contains gas coin + input objects (no execution outputs).
    ///    Then finalize storage (collect storage costs for those objects + budget check).
    /// 2. Charge: apply the gas payment based on the resulting cost summary.
    ///
    /// If the budget check in step 1 fails (computation consumed nearly the entire budget,
    /// leaving no room for even input-object storage), we reset again and set
    /// computation_cost = gas_budget with zero storage. This always succeeds because there
    /// are no storage costs to exceed the budget. Gas coin and input objects remain in the
    /// store so their versions are updated in the transaction effects.
    ///
    ...
    fn reset_and_charge<T, E: ExecutionErrorTrait>(
        gas_charger: &mut GasCharger,
        temporary_store: &mut TemporaryStore<'_>,
        result: &mut Result<T, E>,
    ) -> GasCostSummary {
        gas_charger.reset(temporary_store);
        // v2+: try charging for input-only storage (reduced scope after reset).
        ...
        {
            return gas_charger.charge(temporary_store, result);
        }
        // Input-only storage doesn't fit or v1 path: charge full budget, zero storage.
        gas_charger.reset(temporary_store);
        gas_charger.set_computation_to_budget();
        gas_charger.charge(temporary_store, result)
    }
    ...
    fn execute_ptb<Mode: ExecutionMode>(
        ...
        gas_charger.round_computation(&mut result);
        (result, timings)
    }

    /// Check SUI conservation. On failure, use the uniform reset fork to recover.
    ...
    fn run_conservation_checks<Mode: ExecutionMode>(
        ...
        if let Err(conservation_err) = check_conservation(
            ...
            // Conservation violated. Use the uniform reset fork to recover,
            // then re-check with the new cost summary.
            let mut recovery_result: Result<(), Mode::Error> = Err(conservation_err.into());
            let recovery_cost =
                reset_and_charge(gas_charger, temporary_store, &mut recovery_result);
            ...
            return Err(recovery_result.unwrap_err());
        }
